...
const SatisfactionSurvey = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitted(true);
      toast.success("Obrigado pelo seu feedback! Suas respostas foram salvas.");
    } catch (err) {
      console.error(err);
  ...
  if (isLoading) {
    return (
      <Card className="shadow-2xl border-border/50 overflow-hidden bg-background/80 animate-pulse">
  ...
  if (submitted || hasSubmittedToday) {
    return (
      <Card className="shadow-2xl border-emerald-500/20 bg-emerald-500/5 backdrop-blur-sm overflow-hidden animate-in fade-in zoom-in duration-500">
        <CardContent className="py-16 text-center space-y-4">
          ...
          <p className="text-muted-foreground text-sm max-w-xs mx-auto">
             Você já enviou seu feedback hoje. Obrigado por nos ajudar a melhorar o serviço!
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card className="shadow-2xl border-border/50 overflow-hidden bg-background/80 backdrop-blur-md">
      <CardHeader className="bg-muted/30 py-8 border-b border-border/50">
        ...
        <p className="text-sm text-center text-muted-foreground font-medium max-w-sm mx-auto leading-relaxed">
            Marque seu nível de satisfação em relação ao serviço de alimentação de hoje.
        </p>
      </CardHeader>

      <CardContent className="pt-8 space-y-10 pb-12">
        {/* Table-like Grid for Desktop, Stack for Mobile */}
        <div className="space-y-8">
           {/* Headers (Desktop display: grid) */}
           ...
           {/* Mobile Labels Row (Helpful but maybe too much?) */}
        ...
        <p className="text-[10px] font-black italic text-center text-muted-foreground/60 tracking-widest pt-4">
